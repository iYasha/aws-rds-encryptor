...
class EncryptionPipeline:
    ...
    def create_parameter_group_for_dms(self) -> ParameterGroup:
        migration_parameter_group: ParameterGroup | None = ParameterGroup.from_name(
            name=get_migration_parameter_group_name(self.rds_instance.parameter_group.name)
        )
    ...
    def create_pglogical_extension_in_source_db(self):
        ...
        while "pglogical" not in db_manager.get_parameter(
            ...
            self.logger.error(
                "`pglogical` not found in `shared_preload_libraries`, restart source database to apply it."
            )
            input()
